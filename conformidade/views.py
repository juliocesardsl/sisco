...
class PadraoConformidadeListView(LoginRequiredMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        # Usar o código da rubrica para busca no arquivo Excel
        resultado = comparar_extrator_por_mes(anterior, atual, rubrica.codigo)
        ...
        self.request.session['rubrica_codigo'] = rubrica.codigo
        self.request.session['rubrica_nome'] = rubrica.nome
...
def verificacao_create(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if not arquivo_vencimento or not arquivo_extrator:
                messages.error(request, 'Por favor, selecione ambos os arquivos (Vencimento e Extrator).')
...
def exportar_verificacao_csv(request):
    ...
    response['Content-Disposition'] = 'attachment; filename="verificacao_resultados.csv"'
    
    writer = csv.writer(response)
    writer.writerow(['Empresa', 'CPF', 'Matrícula', 'Nome do Servidor', 'Referência Vertical', 'Referência Horizontal', 'Valor Vencimento', 'Valor Total Recebido', 'Frequência', 'Valor Calculado', 'Diferença (R$)', 'Diferença (%)', 'Status', 'Justificativa'])
    ...
    for item in resultados:
        writer.writerow([
            ...
            f"R$ {item.get('valor_vencimento', '0'):.2f}" if item.get('valor_vencimento') else '',
            f"R$ {item.get('valor_total_recebido', '0'):.2f}" if item.get('valor_total_recebido') else '',
            f"{item.get('frequencia', '')}" if item.get('frequencia') else '',
            f"R$ {item.get('valor_calculado', '0'):.2f}" if item.get('valor_calculado') else '',
            f"R$ {item.get('diferenca_absoluta', '0'):.2f}" if item.get('diferenca_absoluta') else '',
            f"{item.get('diferenca_percentual', '0')}%" if item.get('diferenca_percentual') else '',
            item.get('status', ''),
